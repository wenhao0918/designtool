...
GBSTD_SERVICE = os.environ.get("GBSTD_SERVICE_URL", "http://127.0.0.1:8104")


def dataset_for(category):
    """分类→RAGFlow 库 id:优先 GbstdService(8104) 分类目录;降级 env。

    draw 是默认分类(与 RAGFLOW_DATASET 同库),直接短路;
    其他分类向 GbstdService 查目录,未登记/服务不通 → 回落默认库并告警。
    """
    if not category or category == "draw":
        return RAGFLOW_DATASET
    try:
        req = urllib.request.Request(
            GBSTD_SERVICE + "/api/gbstd/categories")
        with urllib.request.urlopen(req, timeout=5) as r:
            entry = json.loads(r.read().decode()).get(category) or {}
        if entry.get("dataset_id"):
            return entry["dataset_id"]
        print("! 分类 '%s' 未绑定知识库(status=%s),回落默认库"
              % (category, entry.get("status")))
    except Exception as e:
        print("! GbstdService 不可达(%s),分类入库回落默认库" % e)
    return RAGFLOW_DATASET
...
def _multipart_upload(doc_name, content, dataset=None):
    ...
    return _req("POST", "/api/v1/datasets/%s/documents" % (dataset or RAGFLOW_DATASET),
                body=body, is_json=False)


def push_one(code, dataset=None):
    """一节一文档:每节独立上传为单独文档(短文档=单 chunk),
    从机制上杜绝跨条目切块(naive 对长文档会腰斩条款)。
    文档名:GB_T4458.4-2003#01_basic_rules.md
    """
    _ds = dataset or RAGFLOW_DATASET
    d = os.path.join(DOCS, dir_name(code))
    meta_path = os.path.join(d, "meta.json")
    with open(meta_path, encoding="utf-8") as f:
        meta = json.load(f)
    ...
    try:
        ...
        for doc in ((r.get("data") or {}).get("docs") or []):
            if doc.get("name", "").startswith(prefix):
                _req("DELETE", "/api/v1/datasets/%s/documents" % _ds,
                     body={"ids": [doc["id"]]})
    except Exception:
        pass
    doc_ids = []
    for fn in secs:
        ...
        r = _multipart_upload(doc_name, content, dataset=_ds)
        if r.get("code") != 0:
            print("✗ %s 上传失败: %s" % (fn, r.get("message")))
            return False
        doc_ids += [x["id"] for x in (r.get("data") or [])]
    # 触发解析
    _req("POST", "/api/v1/datasets/%s/chunks" % _ds,
...
def cmd_push(code=None, category="draw"):
    stds = parse_catalog()
    n = 0
    for s in stds:
        ...
        if errs:
            ...
            continue
        if push_one(s["code"], dataset_for(category)):
            n += 1
...
def main():
    ...
    ap.add_argument("--category", default="draw", help="国标分类(经 GbstdService 8104 解析目标库)")
    a = ap.parse_args()
    if a.cmd == "init":
        cmd_init(a.force)
    elif a.cmd == "validate":
