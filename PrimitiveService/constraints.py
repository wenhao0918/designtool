...
def validate(parts, joints=None):
    ...
    for p in parts:
        ...
        if not _get(p, "base"):
            add(HARD, name, "hole_base_required",
                "side_hole 缺 base——孔不指定基体则不会切割任何材料(游离圆柱)",
                "base=要开孔的零件名,如 base=底板")
    ...
    # ---- feas:裸 cylinder 当刀具(不切割,静默混入装配) ----
    for p in parts:
        if p.get("type") != "cylinder":
            continue
        n = _part_name(p)
        if any(k in n for k in ("刀具", "tool", "cut")):
            referenced = set()
            for q in parts:
                if q.get("type") == "subtract":
                    referenced.update(_get(q, "tools") or [])
            if n not in referenced:
                add(FEAS, n, "cylinder_as_tool",
                    "圆柱 '%s' 命名为刀具但 cylinder 不会切割任何基体" % n,
                    "改用 side_hole(base=基体名) 直接开孔;或显式 subtract(base=..., tools=[%s])" % n)
